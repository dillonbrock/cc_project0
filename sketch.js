...
function setup() {
  ...
  for (var i = 0; i < valleyHeights.length; i++) {
    ...
    for (var i = 0; i < valleyHeights.length; i++) {
      numberedValleys.push(orderedValleyHeights.indexOf(valleyHeights[i]));
    }
    for (var i = 0; i < peakHeights.length; i++) {
      numberedPeaks.push(orderedPeakHeights.indexOf(peakHeights[i]));
    }
    valleyNumber = Number(numberedValleys.join(''));
  ...
  // shallowest to deepest valley
      ...
      case 12:
        if(itr < height*(1-orderedValleyHeights[1])) {
          ...
          line(0, itr, findxOffset(orderedPeakHeights[numberedPeaks[0]]), itr);
        }
      break;
